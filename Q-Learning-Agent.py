...
def calculate_rewards(game_grid, survived_steps, total_lines_cleared, terminated):
    total_heights, bumpiness, holes, y_pos = compute_grid_features(game_grid)
    ...
    calc_reward += 0.1 * survived_steps
    ...
    calc_reward -= 0.05 * total_heights

    calc_reward += (2 ** total_lines_cleared) * 10
    if total_lines_cleared == 4:
        calc_reward += 5000  # Big reward for clearing 4 lines

    calc_reward += hole_penalty * holes
    calc_reward -= 0.1 * bumpiness
...
n_test_runs = 3  # Display agent n times after training
